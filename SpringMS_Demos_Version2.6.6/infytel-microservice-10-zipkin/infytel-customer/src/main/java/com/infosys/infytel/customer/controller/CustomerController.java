...
//	@Autowired
	...
	public CustomerDTO getCustomerProfile(@PathVariable Long phoneNo) throws InterruptedException, ExecutionException {
		...
		//List<Long> list = hystService.getFriends(phoneNo);
		List<Long> list=template.getForObject("http://FRIENDFAMILYMS"+"/customers/"+phoneNo+"/friends", List.class);
...
//		custDTO.setFriendAndFamily(friendsFuture.get());
		...
		custDTO.setFriendAndFamily(list);
